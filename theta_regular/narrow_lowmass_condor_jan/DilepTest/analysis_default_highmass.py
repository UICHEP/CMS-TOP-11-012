...
def histogram_filter(hname):
    ...
    zp_blacklist = ('zp1000w100', 'zp1500w150', 'zp2000w200', 'zp3000w300',
                    ...
                    'rskkg',  'zp750', 'zp1250', 'kkgluon', )
    for zpw in zp_blacklist:
        if ('__' + zpw) in hname : return False
    return True
 
def histogram_filter_semilep(hname):
    if 'zp' not in hname and 'kkgluon' not in hname and 'rsg' not in hname : return True
    zp_blackList = (
        'wide', 'rsg'
        )
    for zpw in zp_blackList :
    ...
    return m > 500 and m <= 2000 and m % 500 == 0
...
def build_zp_model():
    ...
    modelhadhigh = build_model_from_rootfile(fileshadhigh, histogram_filter, external_to_internal)
    modelhadhigh.fill_histogram_zerobins()
    modelhadhigh.set_signal_processes('zp*')
    for p in modelhadhigh.processes:
       if p=='qcd': continue
       modelhadhigh.add_lognormal_uncertainty('lumi', 0.022, p)
       modelhadhigh.add_lognormal_uncertainty('subjet_scalefactor', math.log(1.06), p)
    modelhadhigh.add_lognormal_uncertainty('ttbar_rate', math.log(1.50), 'ttbar')

    #### Beging semileptonic, high mass ####

    modelsemihigh = build_model_from_rootfile(filessemihigh, histogram_filter_semilep, external_to_internal)
    modelsemihigh.fill_histogram_zerobins()
    modelsemihigh.set_signal_processes('zp*')
    for p in modelsemihigh.processes:
        modelsemihigh.add_lognormal_uncertainty('lumi', math.log(1.022), p)

    # common to e and mu channels
    modelsemihigh.add_lognormal_uncertainty('zl_rate', math.log(2.0), 'zjets')
    modelsemihigh.add_lognormal_uncertainty('vl_rate', math.log(1.5), 'wlight')
    modelsemihigh.add_lognormal_uncertainty('vb_rate', math.log(2.0), 'wb')
    modelsemihigh.add_lognormal_uncertainty('vc_rate', math.log(2.0), 'wc')
    modelsemihigh.add_lognormal_uncertainty('ttbar_rate', math.log(1.15), 'ttbar')
    modelsemihigh.add_lognormal_uncertainty('st_rate', math.log(1.5), 'singletop')

    # mu channels 
    for obs in ('mu_0btag_mttbar', 'mu_1btag_mttbar'):
        for proc in ('wc', 'wb', 'wlight'):
            modelsemihigh.add_asymmetric_lognormal_uncertainty('scale_vjets', -math.log(1.76), math.log(0.62), proc, obs)
            modelsemihigh.add_asymmetric_lognormal_uncertainty('matching_vjets', -math.log(1.08), math.log(0.92), proc, obs)
        for proc in modelsemihigh.processes:
            modelsemihigh.add_lognormal_uncertainty('mu_eff', math.log(1.05), proc, obs)

    # e channels 
    for obs in ('el_0btag_mttbar', 'el_1btag_mttbar'):
        for proc in ('wc', 'wb', 'wlight'):
            modelsemihigh.add_asymmetric_lognormal_uncertainty('scale_vjets', -math.log(1.87), math.log(0.57), proc, obs)
            modelsemihigh.add_asymmetric_lognormal_uncertainty('matching_vjets', -math.log(1.05), math.log(0.95), proc, obs)
        for proc in modelsemihigh.processes:
            modelsemihigh.add_lognormal_uncertainty('ele_eff', math.log(1.05), proc, obs)

    #### End semileptonic, high mass ####
...
model_summary(model)
report.write_html('./mttbar_zp_highmass')
